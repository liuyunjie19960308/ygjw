        ...
        <form class="layui-form">
            ...
            <div class="layui-form-item">
                ...
                <div class="layui-input-inline">
                    <select name="pid">
                        <option selected="selected" value="0">顶级技术</option>
                        <?php foreach($Privilege as $key=>$value): if($value['id']==$list['id']|| in_array($value['id'],$Privilege_id)) continue; ?>
           ...
           <?php if($list['pid']==0):?>
            <div class="layui-upload" style="margin-left: 110px">
                <div class="controls">
                    <?php echo hook('upload',array('is_first'=>1,'unique_sign'=>'icon','field_name'=>'icon','field_value'=>$list['icon'],'save_path'=>'News'));?>
                    <span class="help-block">* 技术图标</span>
                </div>
            </div>
            <?php endif;?>
            <div class="layui-form-item">
                ...
                <div class="layui-input-inline">
                    <input type="text" id="sort" name="sort" required="" lay-verify="sort"
                           autocomplete="off" class="layui-input" value="<?php echo $list['sort'];?>">
                </div>
                <div class="layui-form-mid layui-word-aux">
                    数字升序排序
                </div>
    ...
    <script>
        ...
        layui.use(['form','layer','upload'], function(){
            ...
            form.on('submit(save)', function(data){
                $.post("<?php echo U('Technology/update');?>",$(data.form).serialize(),
                    function(res){
                        if(res.code==200){
                            layer.alert(res.msg, {icon: 6},function () {

                                window.location.href="<?php echo U('Technology/lists');?>";
                            });
                        }
                        if(res.code==201){
