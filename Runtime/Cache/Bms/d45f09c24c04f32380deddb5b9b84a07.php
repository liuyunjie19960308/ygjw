        ...
        <div class="span12">
            <div class="widget-box">
                ...
                <div class="widget-title widget-title-search list-widget-title">
                    <button class="btn btn-success href" url="<?php echo U('Journalism/Journalism_add');?>" target-form="ids">新增</button>&nbsp;
                    <button class="btn btn-inverse ajax-post" url="<?php echo U('Journalism/statusAll',array('value'=>0));?>" target-form="ids">禁用</button>&nbsp;
                    <button class="btn btn-info ajax-post" url="<?php echo U('Journalism/statusAll',array('value'=>1));?>" target-form="ids">启用</button>&nbsp;
                    <button class="btn btn-danger confirm ajax-post" url="<?php echo U('Journalism/deleteAll');?>" target-form="ids">删除</button>&nbsp;
                    ...
                    <table class="table table-bordered table-striped with-check reset-checkbox">
                        ...
                        <?php foreach($list as $v):?>
                            ...
                            <td align="center">
                                <?php echo $v['id'];?>                </td>
                                <td align="center">
                                    <?php echo $v['journalism_title'];?>                </td>


                            <td align="center">
                                <?php echo (get_status_title($v['status'])); ?>
                                <td align="center">
                                    <?php echo $v['add_time']?>                </td>


                                <td>
                                    <a href="<?php echo U('Journalism/desc',array('id'=>$v['id']));?>" title="编辑" class="tip-bottom">
                                        <span class="label label-success label-success-hover">编辑</span></a>&nbsp;
                                    <?php if($v['status'] == 0 or $v['status'] == 1): ?><a href="<?php echo U('Journalism/statusOne',array('value'=>$v['status'],'id'=>$v['id']));?>" title="<?php echo (show_status_name($v['status'])); ?>" class="tip-bottom ajax-get">
                                            <span class="label <?php echo (show_status_label($v['status'])); ?>"><?php echo (show_status_name($v['status'])); ?></span></a>&nbsp;<?php endif; ?>

                                    <a href="<?php echo U('Journalism/deleteOne',array('ids'=>$v['id']));?>" data-ing-html="正在删除 <i class='fa fa-spinner fa-spin'></i>" title="删除" class="tip-bottom confirm ajax-get">
