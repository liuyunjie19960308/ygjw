                ...
                <div class="widget-content no-padding no-border">
                    <table class="table table-bordered table-striped">
                        ...
                        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr>
                                <td class="va-middle"><?php echo ($row['id']); ?></td>
                                <td>
                                    <img src="<?php echo ($row['picture']); ?>" style="width:150px;">
                                </td>
                                <td class="va-middle"><?php echo (get_config_title($row['position'],ADVERT_POSITION)); ?></td>
                                <!-- <td class="va-middle"><?php echo ((isset($row['description']) && ($row['description'] !== ""))?($row['description']):'---'); ?></td> -->
                                <td class="va-middle">
                                    <?php echo (timestamp2date($row['start_time'],'Y-m-d')); ?> ~ <?php echo (timestamp2date($row['end_time'],'Y-m-d')); ?>
                                    <?php if($row['start_time'] < NOW_TIME and $row['end_time'] > NOW_TIME): ?><span class="badge badge-success">投放中</span>
                                    <?php else: ?>
                                        <span class="badge badge-important">已过期</span><?php endif; ?>
                                </td>
                                <td class="va-middle"><?php echo (get_config_title($row['target_rule'],TARGET_RULES)); ?></td>
                                <td class="quick-edit va-middle" data-model="Advert" data-id="<?php echo ($row['id']); ?>" data-field="sort"><?php echo ($row['sort']); ?></td>
                                <td class="va-middle"><?php echo (get_status_title($row['status'])); ?></td>
                                <td class="va-middle">
