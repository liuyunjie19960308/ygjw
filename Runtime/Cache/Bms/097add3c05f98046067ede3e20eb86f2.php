        ...
        <div class="span12">
            <div class="widget-box">
                ...
                <div class="widget-title widget-title-search list-widget-title">
                    <button class="btn btn-success href" url="<?php echo U('EnterpriseContact/EnterpriseContact_add');?>" target-form="ids">新增</button>&nbsp;
                    <!--<button class="btn btn-inverse ajax-post" url="<?php echo U('EnterpriseContact/statusAll',array('value'=>2));?>" target-form="ids">禁用</button>&nbsp;-->
                    <!--<button class="btn btn-info ajax-post" url="<?php echo U('EnterpriseContact/statusAll',array('value'=>1));?>" target-form="ids">启用</button>&nbsp;-->
                    <button class="btn btn-danger confirm ajax-post" url="<?php echo U('EnterpriseContact/deleteAll');?>" target-form="ids">删除</button>&nbsp;
                    <!--<button type="button" class="btn btn-primary move">移动</button>-->
                    <span style="display: none">
                    <?php echo (replace($select,'target-form','ids')); ?><button class="btn btn-primary ajax-post"  target-form="ids">确定</button>
                    </span>
                        ...
                        <?php foreach($list as $v):?>
                            ...
                            <td align="left">
                                ...
                                <td align="left">
                                    <?php echo $v['phone'];?>                </td>
                                <td align="left">
                                    <?php echo $v['customer'];?>                </td>
                                <td align="left">
                                    <?php echo $v['mailbox'];?>                </td>
                                <td align="center">
                                    <?php echo $v['sort'];?>                </td>
                                <td>
                                    <a href="<?php echo U('EnterpriseContact/desc',array('id'=>$v['id']));?>" title="编辑" class="tip-bottom">
                                        <span class="label label-success label-success-hover">编辑</span></a>&nbsp;
                                    <!--<?php if($v['status'] == 2 or $v['status'] == 1): ?>-->
                                        <!--<a href="<?php echo U('EnterpriseContact/statusOne',array('value'=>$v['status'],'id'=>$v['id']));?>" title="<?php echo (show_status_name($v['status'])); ?>" class="tip-bottom ajax-get">-->
                                            <!--<span class="label <?php echo (show_status_label($v['status'])); ?>"><?php echo (show_status_name($v['status'])); ?></span></a>&nbsp;-->
                                    <!--<?php endif; ?>-->

                                    <a href="<?php echo U('EnterpriseContact/deleteOne',array('ids'=>$v['id']));?>" data-ing-html="正在删除 <i class='fa fa-spinner fa-spin'></i>" title="删除" class="tip-bottom confirm ajax-get">
