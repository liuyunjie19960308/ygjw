                ...
                <div class="widget-content no-padding no-border">
                    <table class="table table-bordered table-striped">
                        ...
                        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr>
                                ...
                                <td><?php echo (get_plugins_status_title($row['status'])); ?></td>
                                <td><?php echo ($row['author']); ?></td>
                                <td><?php echo ($row['version']); ?></td>
                                <td>
                                    <?php if(empty($row['uninstall'])): $class = get_plugin_class($row['name']); if(!class_exists($class)) { $has_config = 0; } else { $plugin = new $class(); $has_config = count($plugin->getConfig()); } ?>
                                        <!--存在配置信息 显示设置-->
                                        <?php if ($has_config): ?>
                                            <a href="<?php echo U('config',array('id'=>$row['id']));?>" title="设置" class="">
                                                <span class="label label-hover">设置</span></a>&nbsp;
                                        <?php endif ?>
                                        <?php if ($row['status'] == 0 || $row['status'] == 1): ?>
                                        <a href="<?php echo U('Plugins/forbidResume',array('model'=>'Plugins','ids'=>$row['id'],'field'=>'status','value'=>abs(1-$row['status'])));?>" title="<?php echo (show_status_name($row['status'])); ?>" class="ajax-get">
                                            <span class="label <?php echo (show_status_label($row['status'])); ?>"><?php echo (show_status_name($row['status'])); ?></span></a>&nbsp;
                                        <?php endif ?>
                                        <a class="ajax-get" href="<?php echo U('Plugins/uninstall',array('id'=>$row['id']));?>" title="卸载">
                                            <span class="label label-important label-important-hover">卸载</span></a>&nbsp;
                                    <?php else: ?>
                                        <a class="ajax-get" href="<?php echo U('Plugins/install',array('plugin_name'=>$row['name']));?>" title="安装">
                                            <span class="label label-success label-success-hover">安装</span></a><?php endif; ?>
                                </td>
