    ...
    $('td.quick-edit').dblclick(function () {
        ...
        if(value.match(new RegExp(/<[^>]+>/g)) == null) {
            var html = '<input type="text" style="margin-bottom:0;width:' + width + 'px" data-old-value="' + value + '" ' +
                'data-controller="' + that.attr('data-controller') + '" data-model="' + that.attr('data-model') + '" data-field="' + that.attr('data-field') + '" data-id="' + that.attr('data-id') + '" ' +
...
function quick_edit_input_blur(obj) {
    var that = $(obj), controller = that.attr('data-controller'), model = that.attr('data-model'), field = that.attr('data-field'), id = that.attr('data-id'),
        new_value = that.val(), old_value = that.attr('data-old-value');
    if(controller == 'undefined' || typeof(controller) == 'undefined') {
        controller = model;
    }
    var target = '/'+controller+'/quickEdit', data = {model:model,ids:id,field:field,value:new_value};
...
function quick_edit_select_change(obj) {
    var that = $(obj), controller = that.attr('data-controller'), model = that.attr('data-model'), field = that.attr('data-field'),
        id = that.attr('data-id'), value = that.val();
    if(controller == 'undefined' || typeof(controller) == 'undefined') {
        controller = model;
    }
    var target = '/'+controller+'/quickEdit', data = {model:model,ids:id,field:field,value:value};
    $.post(target,data).success(function(data){
        if (data.status == 1) {
...
function getSmsVerify(account, unique_code, target, that) {
    if(account == '') {
        updateAlertWithTime('请输入手机号','alert-error', 1500); return;
    }
    $.post(target, {account:account,unique_code:unique_code}).success(function(data) {
        if (data.status == 1) {
            time(that);
        } else {
            updateAlertWithTime(data.info,'alert-error', 1500); return;
        }
    });
}