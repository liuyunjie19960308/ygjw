...
class BmsBaseController extends MscBaseController {
    ...
    public function update() {
        ...
        if(!IS_POST) {
            ...
            if($result) {
                $this->success(self::$logicObject->getLogicInfo(), self::$forward == '' ? cookie('__forward__') : self::$forward);
            } else {
                $this->error(self::$logicObject->getLogicInfo());
            }
        }
    ...
    function json_retrun($code, $msg = "", $data = [],$count=0)
    {
        if(!empty($count)){
            $result = array(
                'code' => $code,
