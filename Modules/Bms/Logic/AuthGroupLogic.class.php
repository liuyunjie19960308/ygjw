...
class AuthGroupLogic extends BmsBaseLogic {
    ...
    function getList($request = array()) {
        ...
        //查询的字段
        $param['field']     = 'id,name,description,status';
        //返回数据
    ...
    function getAccess($request = array()) {
        ...
        if(!$list) {
            //获取所有权限规则
            $list = D('AuthRule')->field('id,rule,name,parent_id')->where(array('status'=>1))->select();
            //处理成树状结构
