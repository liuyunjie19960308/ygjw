 ...
 * 
 */
class ReviewLogic extends BmsBaseLogic
{
    ...
    public function getList($request = [])
    {
        ...
        // 返回数据
        $result = D('Review')->getList($param);

        foreach ($result['list'] as &$value) {
            $value['cover'] = api('File/getFiles', array($value['cover']));
        }
    ...
    public function findRow($request = [])
    {
        ...
        // 获取数据
        $row = D('Review')->findRow($param);
        if (!$row) {
            return $this->setLogicInfo('未查到此记录！', false);
        }
        // 获取展示封面
        $row['cover'] = api('File/getFiles', [$row['cover']]);
        // 获取视频连接 file_url
