<?php
namespace Bms\Model;
class TechnologyModel extends BmsBaseModel
{
//
//    /**
//     * @var array
...
//                echo $this->getLastSql();die;
     ...
     * @return string
     */
    public function model_status($value,$id=[]){
        if ($value==0){
            $res=$this->where($id)->save(['status'=>1]);
        }  elseif ($value==1){
            $res=$this->where($id)->save(['status'=>0]);
        }
        return $res;
    }

    public  function get_tree($where,$parent_id = 0){

        $data=$this->alias('a')->field('a.*,v.name as pri_name')
            ->where($where)
            ->join(" LEFT JOIN toocms_Technology v ON a.pid= v.id")
    ...
    public function _tree($data,$parent_id=0,$level=0){
        ...
        foreach ($data as $d ){
            ...
            if ($d['pid'] == $parent_id){
                $d['level'] = $level; //标志这是几级的，变量$level
                $arr[] = $d;


                $this->_tree($data,$d['id'],$level + 1);
            }
    ...
    //找自己的子分类
    public  function category_id($parent_id){

        $data=$this->select();
        return $this->get_category_id($data,$parent_id);
    }

    private function get_category_id($data,$parent_id){
        $arr=array();
