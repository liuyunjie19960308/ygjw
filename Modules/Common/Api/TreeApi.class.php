...
class TreeApi {
    ...
    public static function toFormatTree($tree = array(), $level = 0) {
        ...
        foreach ($treeData as $data) {
            ...
            foreach(self::$_fields as $field) {
                ...
                if(is_array($field)) {
                    //替换数据调用方法转换
                    $value = $field[1]($data[$field[0]], $data);
                    //获取字段名称
                    $field = $field[0];
                } else {
                    $value = $data[$field];
    ...
    public static function getAllParent($list = [], $root = 0, $pk = 'id', $pid = 'parent_id') {
        ...
        //return array_reverse(self::$_singleTree);  array_reverse返回一个单元顺序相反的数组
        return array('ids' => array_column(self::$_singleTree, $pk), 'list' => self::$_singleTree);
    }
