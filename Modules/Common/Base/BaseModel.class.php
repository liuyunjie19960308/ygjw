...
class BaseModel extends Model {
     ...
     * 获取数据库连贯操作
     */
    final private function _getCURD($param = [], $Page = null) {
        $model = $this;
        //表别名 字符串
     ...
     */
    protected function getPage($total = 0, $page_size = 0, $parameter = []) {
        //初始化分页对象
    ...
    protected function checkUnique($value, $field) {
        ...
        if(!empty($_REQUEST[$pk])) {
            //如果原值等于现值  不在进行下一步判断
            if($this->where([$pk => $_REQUEST[$pk]])->getField($field) == $value)
                return true;
        }
        //判断现值是否存在
        if($this->where(['' . $field . '' => $value])->count() > 0)
            return false;
        return true;
    }
     ...
     */
    protected function specialSearch($param, $Page = null, $alias = '', $join = []) {
        //数据对象
        $model                          = $this;
        //要查询的字段 此方法只查询ID
        $field                          = 'id';
        ...
        //echo $model->_sql();
        //数组降维
        $ids                            = array_column(array_merge($id_list, [['id'=>0]]), 'id');
        //查询ID 在数组中的结果集
        $param['special_where'][$field] = ['exp', ' IN (' . implode(',', $ids) . ')'];
        //结果数组排序时使用
