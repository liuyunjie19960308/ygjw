...
function get_status_title($status = '') {
    switch ($status) {
        ...
        case 1  : return    '<span style="color:#E5504D">禁用</span>';   break;
        case 2  : return    '拒绝通过'; break;
        case 3  : return    '<span style="color:#007DCC">审核中..</span>'; break;
...
function get_region_type_name($type) {
    switch ($type) {
        case 1       : return    '省';    break;
        case 2       : return    '城市';    break;
        case 3       : return    '区县';    break;
        case 4       : return    '商圈';    break;
        default      : return    '--';       break;
    }
}
 ...
 * 获取地区名称
 */
function get_region_name($region_id, $reg_type = 1) {
    if(empty($region_id))
        return $reg_type == 1 ? '--选择城市--' : $reg_type == 2 ? '--选择区域--' : '--选择商圈--';
    //获取地区名称
    $name = M('Region')->where(['id'=>$region_id])->getField('name');
    if(!$name)
        return $reg_type == 1 ? '--选择城市--' : $reg_type == 2 ? '--选择区域--' : '--选择商圈--';
    else
        return $name;
