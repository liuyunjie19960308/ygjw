 ...
 * 
 */
class TraineesLogic extends FrontBaseLogic
{
    ...
    {
        // 获取数据
        $result = D('FrontC/Trainees', 'Service')->getList($param, $request);
    ...
    {
        if (empty($request['trainees_id'])) {
            return $this->setLogicInfo('参数错误！', false);
        }
        // 查询条件
        $param['where']['trainees.id'] = $request['trainees_id'];
        // 查找
        $row = D('FrontC/Trainees')->findRow($param);
